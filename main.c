...
#define client_fifo "C:/Users/rodol/Desktop/isec_1819/summerInovationWeek"
#define server_fifo "C:/Users/rodol/Desktop/isec_1819/summerInovationWeek"
...
int main(int argc, char * argv[])
{
  ...
  while(run_flag)
  {
    ...
    for(index = 0; index != blocks_copied; ++index) {    
       ...
       //printf("  %s\n", buf);
       sprintf(aux, "%d;%d;%d;%d;%d\n", blocks.signature, blocks.height, blocks.width, blocks.x, blocks.y);
       strcat(info, aux)
    }
    strcat(info, "\0");
    ret1 = write(s_fd, &info, (char) 1024);
    i++;
  }
  pixy_close();
}